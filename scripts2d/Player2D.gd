...
func _ready() -> void:
	...
	collision_mask = 3
	max_speed = GameState.fleet_speed() * K * GameState.formation_passive("speed") * GameState.badge_mult("speed")   # #196/#224再2: 単縦陣で+5%
	_build_visual()
...
func _physics_process(delta: float) -> void:
	...
	if charge_t > 0.0:
		...
		collision_mask = 1          # #224再5: 突撃中は敵(2)を外して貫通
...
func _charge_pierce() -> void:
	...
	for e in get_tree().get_nodes_in_group("enemy"):
		...
		var dmg := rd * (0.5 + velocity.length() / maxf(max_speed, 1.0)) * GameState.formation_passive("ram") * GameState.badge_mult("ram") * wedge_mult
...
func _handle_ram() -> void:
	...
	for e in get_tree().get_nodes_in_group("enemy"):
		...
		var ram_dmg := rd * (0.5 + velocity.length() / maxf(max_speed, 1.0)) * GameState.formation_passive("ram") * GameState.badge_mult("ram")   # #224再2: 鋒矢陣
