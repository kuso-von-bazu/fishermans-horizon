...
func _ready() -> void:
	...
	var out := "| 武器 | 種別 | 単発火力 | 連射間隔(秒) | 毎秒火力 | 弾数 | リロード(秒) | 1弾倉の総火力 | 弾速(px/秒) | 射程 | 価格 | 販売島 |\n"
	out += "|---|---|---|---|---|---|---|---|---|---|---|---|\n"
	for wid in Database.weapons:
		...
		# 弾速は Projectile2D.setup と同じ式 (70 + 単発火力*0.3) * K(6) * 弾速倍率
		var shot_speed: float = (70.0 + dmg * 0.3) * 6.0 * float(w.get("speed_mult", 1.0))
		out += "| %s | %s | %.0f | %.2f | %.0f | %d | %.1f | %.0f | %.0f | %d | %d | %s |\n" % [
			str(w.name), ("エイム" if str(w.kind) == "aim" else "ロックオン"),
			dmg, cd, dps, mag, rl, mag_total, shot_speed, int(w.range), int(w.price), where]
	out += "\n(毎秒火力=単発火力÷連射間隔、1弾倉の総火力=単発火力×弾数、弾速は発射直後の基準値。魚雷系は追尾中に0.45〜1.9倍へ変化し、放射系はリーチ480pxで消える)\n"
